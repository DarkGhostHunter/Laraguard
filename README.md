...
* [Laravel 8.75 or later](https://github.com/laravel/framework/blob/8.x/CHANGELOG-8.x.md#v8750-2021-12-04)
* PHP 8.1
...
This package adds a **Contract** to detect if, after the credentials are deemed valid, should use Two-Factor Authentication as a second layer of authentication. Two Factor data is saved **in your application database**, encrypted.
...
public function prepareTwoFactor()
{
...
Then, the User must confirm the Shared Secret with a Code generated by their Authenticator app. The `TotpEnableRequest` request will automatically enable if the code is valid, or returns a validation error if it's not.

```php
use DarkGhostHunter\Laraguard\Http\Requests\TotpEnableRequest;

public function confirmTwoFactor(TotpEnableRequest $request)
{
    return view('user.settings.2fa.success');
}
```

Alternatively, you can use the `confirmTwoFactorAuth` of the user and pass the input value. This way you can tell the User to double-check its device's timezone, or create another Shared Secret with `createTwoFactorAuth()`.
...
You can show them using `recoveryCodes()` of the request instance, or `getRecoveryCodes()` method of the user.
...
> These Recovery Codes are handled automatically when the User sends one instead of a TOTP code. If it's a recovery code, the package will mark it as used.
...
If [safe devices](#safe-devices) is activated, you can "remember" the device after a TOTP authentication when the user authenticates filling the `safe_device` input.

You can change the name of the input using a second parameter.
...
public function login(Request $request)
{
    ...
    $success = Auth::attemptWhen(
        $credentials,
        Laraguard::hasCode(safeDeviceInput: 'remember_2fa_device'),
...
You can deactivate Two-Factor Authentication for a given User using the `disableTwoFactorAuth()` method. This will automatically invalidate the authentication data, safe devices and recovery codes, allowing the User to log in with just his credentials.
...
> For added security, it also cycles the authentication key in the database.
...
If you need to ensure the User has Two-Factor Authentication enabled before entering a given route, you can use the `2fa.enabled` middleware. Since this middleware only checks if 2FA is enabled, it **won't ask the user for any codes**.
